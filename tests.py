...
class TestLight(unittest.TestCase):
    ...
    def test_ww(self, mock_connect, mock_read, mock_send):
        ...
        def read_data(expected):
            ...
            if calls == 2:
                self.assertEqual(expected, 12)
                return bytearray(b"#a&\x10\xb6\x00\x98\x00\x04\x00\xf0\xc2")
            if calls == 3:
                self.assertEqual(expected, 14)
                return bytearray(b"\x81E#a&\x10\x00\x00\x00\x19\x04\x00\x0f\xac")
        ...
        self.assertEqual(
            light.__str__(),
            "ON  [Color: (182, 0, 152) Brightness: 182 raw state: 129,69,35,97,38,16,182,0,152,0,4,0,240,194,]",
        )
        self.assertEqual(light.protocol, PROTOCOL_LEDENET_8BYTE)
        self.assertEqual(light.is_on, True)
        self.assertEqual(light.mode, "color")
        ...
        self.assertEqual(
            light.__str__(),
            "ON  [Warm White: 9% raw state: 129,69,35,97,38,16,0,0,0,25,4,0,15,172,]",
        )
        self.assertEqual(light.protocol, PROTOCOL_LEDENET_8BYTE)
        self.assertEqual(light.is_on, True)
        self.assertEqual(light.mode, "ww")
    ...
    def test_state_transition(self, mock_connect, mock_read, mock_send):
        ...
        def read_data(expected):
            ...
            if calls == 2:
                self.assertEqual(expected, 12)
                return bytearray(b"#a#\x10g\xffh\x00\x04\x00\xf0\x3f")
            if calls == 3:
                self.assertEqual(expected, 14)
                return bytearray(b"\x81E#a#\x10\x01\x19P\x00\x04\x00\xf0\xdb")
            if calls == 4:
                self.assertEqual(expected, 14)
                return bytearray(b"\x81E#a#\x10\x01\x19P\x00\x04\x00\xf0\xdb")
        ...
        self.assertEqual(
            light.__str__(),
            "ON  [Color: (1, 25, 80) Brightness: 80 raw state: 129,69,35,97,35,16,1,25,80,0,4,0,240,219,]",
        )
        self.assertEqual(light.protocol, PROTOCOL_LEDENET_8BYTE)
        self.assertEqual(light.is_on, True)
        self.assertEqual(light.mode, "color")
